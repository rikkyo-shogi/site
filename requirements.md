...
| **社団戦 個人レーティング推移（検証中・専用ページ）** | 紫龍会・立教大学紫龍会の**出場者の個人レーティング（東将連公式・新持点）の推移を表示する**機能。初回実装は「もう少し工夫できそう」とのフィードバックで撤回（2026-07-18）ののち、構成・データ範囲を見直して再実装（2026-07-29、本人同意分＝久保田耕介、2026-07-30に2名追加）。データは `scraper/build_shadan_players.py` が第15〜30回（H16〜R01、東将連サイトのHTML版「参加者全員のランキング」。第21回以降は最終節が複数ファイルに分割されている回があり、対象者ごとに実際に掲載されているパートを資料URLとして使う）・第32回（R05、全部門一覧PDF）・第33回（R06、全部門一覧ランキングPDF）・第34回（R07、部別/全部門ランキングPDF）から登録番号で抽出し `data/shadan/players/*.json` に生成（`player.schema.json`＋CI検証）。掲載者のファイル名（`player_id`）は実名が推測できないよう登録番号由来にする（`kubota-kosuke` は先行実装分でそのまま維持、以降の追加分は `player-<reg_no>` 形式）。第31・32回（R04・R05）は欠測で、内訳は2種類。第31回は個人ランキング資料自体が見当たらず（`31/rankingA4.pdf`・`31/rankingB4.pdf`・`31/league2022_00.pdf` はいずれもチーム成績表で登録番号・持点・通算対局数の列を持たない）＝誰にとっても欠測の**ハード欠測**。第32回は一覧PDF（`32/32_ranking_1022.pdf`）自体は存在し立教の各チームの部員も掲載されているが本人の行がない＝**ソフト欠測**。なお同PDFは所属部・チーム名の列が翌回（第33回）の登録内容になっていることが判明した（2026-09、`32_ichiran_04.pdf`・`data/shadan/confirmed/R05.json`／`R06.json`との突き合わせで確認）。持点・通算対局数はそのまま第32回時点の値として採用し、金子靖史は所属部のみ実際の値（4部赤）へ上書き（`build_shadan_players.py`の`DIVISION_OVERRIDES`）、矢島遼大は紫龍会（OB）が第32回不参加のため当該行を第32回のデータから除外した（ソフト欠測扱いに変更）。第35回（R08、開催中）は東将連が掲載している最新節のランキングを反映する（2026-07-30に第1節時点 `35/35_ranking_01_all.pdf` で開始、2026-09-15に第2節時点 `35/35_ranking_02_all.pdf` へ差し替え。依頼によりシーズン途中でも反映。season_labelに「(第N節時点)」と明記し確定値ではないことを示す。最終節が確定したら通常の season_label に更新する）。表示は `ShadanPlayerRating.astro`：データ点が年1回・欠測ありのため Chart.js ではなくビルド時計算の静的SVG（x軸=回数に比例配置）。隣り合う点の接続は欠測の種類で3通り: 回が連続＝実線＋面塗り、間の回がすべてハード欠測＝接続しない、ソフト欠測を含む＝点線（面塗りなし・参考扱い）。現データでは第30→33回の間に第31回（ハード）と第32回（ソフト）が混在するため点線でつなぐ。各点に回・年度に加え**所属リーグ（部）**をラベル表示。掲載は本人の同意を得た部員のみ・登録番号（`reg_no`）非表示の方針（§5）を維持。**社団戦結果ページ（`shadan/index.astro`）やナビからはリンクしない独立ページ** `site/src/pages/shadan/<ランダム文字列>.astro`（`noindex`・サイトマップ除外）として検証段階の間は非公開運用 |
...
│     │  ├─ shadan/<ランダム文字列>.astro  # 個人レーティング推移(検証用・未リンク・noindex) ✓
...
- `site/src/layouts/BaseLayout.astro` に gtag.js を追加。`GA_MEASUREMENT_ID` 定数を
  ...
  （index / result / archives / shadan / shadan/<ランダム文字列>）に自動で適用される
