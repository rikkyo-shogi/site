...
| **昇降級推移グラフ（ROADMAP §2-3）** | `loadData.ts` の `loadLeagueTrend()` で `data/confirmed` を集計。`LeagueTrend.astro`（Chart.js の stepped line）を大会結果ページ冒頭に配置。縦軸=リーグ名（C2〜A）、横軸=シーズン（H21春〜R08春）。昇級▲/降級▼/優勝★をマーカーで強調。データの無い半期（R02・H21秋 等）は x軸に残しつつ `spanGaps:true` で線を直線接続して表示（マーカーなし。フィードバックにより欠測を目立たせない方針に確定）。直近結果が昇級/降級の場合は次半期の所属リーグ（予定）を点線＋中抜きマーカーで表示（例: R08春昇級→R08秋 B1(予定)。データから自動導出）。JS無効時は簡易テーブルにフォールバック。追加データ収集ゼロ |
...
| **社団戦 歴代成績（第22回〜33回）** | PDFレイアウトが回ごとに異なるため（3リーグ制・3段組・オンライン開催等）、`scraper/build_shadan_history.py` に**目視確認済みの確定値**を保持しスキーマ検証つきで `data/shadan/confirmed/{H23..R06}.json` を生成（半自動＋目視方式）。紫龍会=第22回から参戦（第28〜32回は休会、第33回に5部で復帰）、立教大紫龍会=第27回から参戦（第32回から「立教大学紫龍会」表記）。R02・R03の団体戦はコロナ禍で中止、第31回はR04にオンライン併用の単一リーグ制で開催（順位一覧が無いため `31/rankingA4.pdf` のリーグ表から取得）。昇降欄の無い回の昇級/降級は翌回の所属部から判定し note に根拠を記録。shadanページに歴代成績テーブルを追加 |
| **社団戦 個人レーティング推移（ROADMAP §2-2 の一部）** | **本人の同意（依頼）を得た部員のみ** `scraper/build_shadan_players.py` で抽出し `data/shadan/players/*.json` にコミット（`player.schema.json`＋CI検証。同意のない個人データは従来どおり `data/auto/shadan/` のみ・非コミット）。登録番号は内部キーとしてデータに保持するがページには表示しない。公開レーティング（新持点）が存在するのは第33回（全部門一覧）以降のみ。第1号として久保田耕介（第33回 R1574→第34回 R1635）を shadan ページに Chart.js 折れ線で表示 |
| **SNSリンク** | X / Instagram / note（いずれも rikkyoshogi）へのリンクを `BaseLayout.astro` のフッターに全ページ共通で表示 |
